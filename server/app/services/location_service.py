...
async def _should_refresh_cache() -> bool:
    ...
    try:
        ...
        logger.debug(
            "Checking if location cache needs refresh", extra={"month": current_month}
        )

        # Check if we have a cache for the current month
        cache_doc = await LocationCache.find_one(
            LocationCache.cached_month == current_month
        )

        if cache_doc is None:
            logger.info(
                "No cache document found for current month, will fetch fresh data",
        ...
        logger.debug(
            "Cache is valid, loading from MongoDB",
            extra={"month": current_month, "entry_count": entry_count},
        )
        return False
...
async def _load_cache_from_db() -> bool:
    ...
    try:
        ...
        cache_docs = (
            await LocationCache.find(LocationCache.cache_key == CACHE_KEY)
            .sort("-cached_month")
            .limit(1)
            .to_list()
        )
        ...
        logger.info(
            "Loaded location cache from MongoDB",
            extra={
                "month": cache_doc.cached_month,
                "entry_count": len(_all_locations_cache or []),
            },
        )
        return True
    except Exception as e:
...
async def _save_cache_to_db():
    ...
    try:
        ...
        existing_doc = await LocationCache.find_one(
            LocationCache.cached_month == current_month
        )
        ...
        if existing_doc:
            ...
            logger.info(
                "Updated existing location cache",
                extra={
                    "month": current_month,
                    "entry_count": len(_all_locations_cache or []),
                },
        ...
        if len(all_caches) > 2:
            ...
            for old_cache in all_caches[2:]:
                logger.debug(
                    "Deleting old location cache",
                    extra={"month": old_cache.cached_month},
                )
...
async def _build_all_locations_cache_from_api() -> list[dict]:
    ...
    logger.info("Location cache built from API", extra={"entry_count": len(locations)})
    return locations
...
async def search_locations(query: str, limit: int = 20) -> list[dict]:
    ...
    results.sort(
        key=lambda x: (
            x[0],
            type_priority.get(x[1].get("type", ""), 3),
            x[1].get("name", ""),
        )
    )
    ...
    logger.debug(
        ...
        extra={"query": query, "result_count": len(results[:limit])},
    )
...
async def preload_cache():
    ...
    try:
        ...
        logger.info(
            "Location cache preload complete",
