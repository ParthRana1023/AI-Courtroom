...
async def list_cases(current_user: User = Depends(get_current_user)):
    ...
    cases = await Case.find(
        Case.user_id == current_user.id, {"is_deleted": {"$ne": True}}
    ).to_list()
...
async def update_case_status(
    ...
    if new_status == CaseStatus.ACTIVE.value and (
        not case.user_role or case.user_role == Roles.NOT_STARTED
    ):
        raise HTTPException(
            status_code=400,
...
async def generate_plaintiff_opening(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to generate opening statement. Please try again.",
        )
...
async def restore_case(cnr: str, current_user: User = Depends(get_current_user)):
...
async def get_case_history(
    case_identifier: str, current_user: User = Depends(get_current_user)
):
