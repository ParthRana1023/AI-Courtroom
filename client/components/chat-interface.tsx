...
export function ChatInterface({ caseId, role, caseData }: ChatInterfaceProps) {
  ...
  // Update the handleSubmit function to match the backend API
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If there's a verdict, add it as a judge message
      if (response.isVerdict) {
          ...
          >
            ...
            >
              <Avatar
                className={cn(
                  "h-8 w-8",
                  message.role === "judge" && "bg-yellow-100"
                )}
              >
                <AvatarFallback>
                  {message.role === "user"
      ...
      <div className="border-t p-4">
        <form onSubmit={handleSubmit} className="flex gap-2">
          ...
          <Button
            type="submit"
            size="icon"
            disabled={!input.trim() || isSubmitting || isComplete}
          >
            {isSubmitting ? (
        ...
        {isComplete && (
          <p className="text-sm text-muted-foreground mt-2 text-center">
            This argument phase is complete. The final verdict has been
            rendered.
          </p>
        )}
      </div>
